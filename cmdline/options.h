...
public:
   OptArgvIter( char *  argv[])
      : av(argv), ac(-1), ndx(0) {}

   OptArgvIter(int argc,  char *  argv[])
...
private:
   unsigned     len;        // length of token-string
    ...
    char * seps;       // delimiter-set (separator-characters)
    char * cur;        // current token
   char       * tokstr;     // our copy of the token-string

   static  char * default_delims;  // default delimiters = whitespace

public:
   OptStrTokIter( char * tokens,  char * delimiters =0);
...
// DESCRIPTION
// ===========
// The Options ructor expects a command-name (usually argv[0]) and
...
private:
   ...
   unsigned       optctrls : 7;  // control settings (a set of OptCtrl masks)
    char  *  * optvec; // vector of option-specifications (last=NULL)
    char   * nextchar;      // next option-character to process
    char   * listopt;       // last list-option we matched
   ...
   check_syntax(void) ;
    ...
    char *
   match_opt(char opt, int ignore_case =0) ;

    char *
   match_longopt( char * opt, int  len, int & ambiguous) ;

   int
   parse_opt(OptIter & iter,  char * & optarg);

   int
   parse_longopt(OptIter & iter,  char * & optarg);
...
public:
   ...
   void
   usage(ostream & os,  char * positionals)  ;
   ...
   int
   operator()(OptIter & iter,  char * & optarg) ;
      ...
      //
   int
   explicit_endopts()  { return  explicit_end; }
} ;
