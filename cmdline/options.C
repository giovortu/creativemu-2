...
static  char ident[] = "@(#)Options  1.05" ;
...
public:
   ...
   ostream &
   write( char * buf, unsigned bufsize);
...
ostream &
ostream::write( char * buf, unsigned ) {
   fputs(buf, fp);
   return *this;
}
...
static  char WHITESPACE[] = " \t\n\r\v\f" ;
 char * OptStrTokIter::default_delims = WHITESPACE ;

OptStrTokIter::OptStrTokIter( char * tokens,  char * delimiters)
...
// ************************************************************* OptIstreamIter
 ...
 unsigned  OptIstreamIter::MAX_LINE_LEN = 1024 ;
...
// ^^-------------------------------------------------------------------------
static kwdmatch_t
kwdmatch( char * src,  char * attempt, int len =0) {
   int  i;
...
// **************************************************************** OptionSpec
   ...
   //    *NOTE*:: Assumes that the char-ptr given to the ructor points
   //             to storage that will NOT be modified and whose lifetime will
   //             be as least as long as the OptionSpec object we ruct.
   //
class OptionSpec {
public:
   OptionSpec( char * decl =NULLSTR)
      : hidden(0), spec(decl)
   {
   ...
   OptionSpec( OptionSpec & cp) : hidden(cp.hidden), spec(cp.spec) {}

   // NOTE: use default destructor!

      // Assign to another OptionSpec
   OptionSpec &
   operator=( OptionSpec & cp) {
      if (this != &cp) {
         spec = cp.spec;
         hidden = cp.hidden;
      }
   ...
   operator  char*() { return  isHiddenOpt() ? (spec - 1) : spec; }

      // Is this option NULL?
   int
   isNULL(void)  { return ((spec == NULL) || (spec == NULL_spec)); }

      // Is this options incorrectly specified?
   int
   isSyntaxError( char * name) ;

      // See if this is a Hidden option
   int
   isHiddenOpt(void)  { return  hidden; }

      // Get the corresponding option-character
   char
   OptChar(void)  { return  *spec; }
   ...
   unsigned
   Format(char * buf, unsigned optctrls) ;
...
private:
   ...
   unsigned     hidden : 1;  // hidden-flag
    char * spec;        // string specification

   static  char NULL_spec[];
} ;

 char OptionSpec::NULL_spec[] = "\0\0\0" ;

int
OptionSpec::isSyntaxError( char * name)  {
   int  error = 0;
   if ((! spec) || (! *spec)) {
...
//
// ^SYNOPSIS:
//    unsigned OptionSpec::Format(buf, optctrls) 
//
// ^PARAMETERS:
...
// ^^-------------------------------------------------------------------------
unsigned
OptionSpec::Format(char * buf, unsigned optctrls)  {
#ifdef NO_USAGE
   return  (*buf = '\0');
#else
...
Options::~Options(void) {}
   ...
   //
void
Options::check_syntax(void)  {
   int  errors = 0;
   if ((optvec == NULL) || (! *optvec))  return;

   for ( char *  * optv = optvec ; *optv ; optv++) {
...
//
// ^SYNOPSIS:
//     char * match_opt(opt, int  ignore_case) 
//
// ^PARAMETERS:
...
// ^^-------------------------------------------------------------------------
 char *
Options::match_opt(char opt, int ignore_case)  {
...
//
// ^SYNOPSIS:
//    char * Options::match_longopt(opt, len, ambiguous)
//
// ^PARAMETERS:
...
// ^^-------------------------------------------------------------------------
 char *
Options::match_longopt( char * opt, int  len, int & ambiguous)  {
   kwdmatch_t  result;
    char * matched = NULLSTR ;
...
//     char * & optarg -- where to store any option-argument
//
// ^DESCRIPTION:
...
// ^^-------------------------------------------------------------------------
int
Options::parse_opt(OptIter & iter,  char * & optarg) {
...
// ^^-------------------------------------------------------------------------
int
Options::parse_longopt(OptIter & iter,  char * & optarg) {
   ...
   if (val) {
      ...
      // Check for argument in next arg
    char * nextarg = iter.curr();  // find the next argument to parse
   if ((nextarg != NULL)  &&
       (optspec.isValRequired() || (! isOption(optctrls, nextarg)))) {
...
// ^^-------------------------------------------------------------------------
void
Options::usage(ostream & os,  char * positionals)  {
#ifdef NO_USAGE
   return;
#else
    char *  * optv = optvec;
...
// ^^-------------------------------------------------------------------------
int
Options::operator()(OptIter & iter,  char * & optarg) {
