...
void sn76496Write(int chip, int data)
{
  ...
  /* update buffer before register changes */
  // In a real implementation, we'd update here. 
  // For now, ensure the register changes are atomic and effective immediately.
  ...
  while (length > 0) {
    ...
    do {
      ...
      if (R->Count[3] <= 0) {
        ...
        // Ensure Period[3] is at least a reasonable value to prevent excessive iterations
        if (R->Period[3] < 16) R->Period[3] = 16;
    ...
    int64_t out_val = (int64_t)(2 * vol[0] - STEP) * R->Volume[0] +
                      (int64_t)(2 * vol[1] - STEP) * R->Volume[1] +
                      (int64_t)(2 * vol[2] - STEP) * R->Volume[2] +
                      (int64_t)(2 * vol[3] - STEP) * R->Volume[3];
    *(buffer++) = (int16_t)(out_val / STEP);
    length--;
  }
}
...
static void sn76496_set_clock(int chip, int clock)
{
  ...
  /* number. */
  R->UpdateStep = ((double)STEP * R->SampleRate * 16.0) / clock;
}
...
static void sn76496_set_gain(int chip, int gain)
{
  ...
  /* Base output is MAX_OUTPUT / 4 to prevent clipping when all 4 channels are active. */
  /* We ignore the 'gain' parameter here to prevent destroying the volume envelope. */
  out = MAX_OUTPUT / 4.0;
...
int sn76496Init(int chip, int clock, int gain, int sample_rate)
{
  ...
  for (i = 0; i < 8; i += 2) {
    R->Register[i] = 0;
    R->Register[i + 1] = 0x00;  /* volume = 0 (max volume) */
  }
